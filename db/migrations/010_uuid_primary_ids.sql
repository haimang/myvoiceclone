-- Migration 010: replace remaining integer autoincrement ids with mvc_ UUID text ids

PRAGMA foreign_keys=off;

CREATE TABLE IF NOT EXISTS job_events_uuid (
    ...
    CONSTRAINT check_job_events_id_mvc CHECK (id LIKE 'mvc_%')
);

INSERT INTO job_events_uuid (
    id, job_id, event_type, status_from, status_to, message, created_at, metadata_json
)
SELECT
    CASE WHEN typeof(id) = 'text' AND id LIKE 'mvc_%' THEN id ELSE 'mvc_' || lower(hex(randomblob(16))) END,
    job_id, event_type, status_from, status_to, message, created_at,
    COALESCE(metadata_json, '{}')
FROM job_events;

DROP TABLE job_events;
ALTER TABLE job_events_uuid RENAME TO job_events;

CREATE INDEX IF NOT EXISTS idx_job_events_job ON job_events(job_id);
CREATE INDEX IF NOT EXISTS idx_job_events_type ON job_events(job_id, event_type);

CREATE TABLE IF NOT EXISTS eval_metrics_uuid (
    id TEXT PRIMARY KEY DEFAULT ('mvc_' || lower(hex(randomblob(16)))),
    ...
    CONSTRAINT check_eval_metrics_id_mvc CHECK (id LIKE 'mvc_%')
);

INSERT INTO eval_metrics_uuid (
    id, run_id, report_id, metric_name, metric_value, metric_json, step, created_at
)
SELECT
    CASE WHEN typeof(id) = 'text' AND id LIKE 'mvc_%' THEN id ELSE 'mvc_' || lower(hex(randomblob(16))) END,
    run_id, report_id, metric_name, metric_value, COALESCE(metric_json, '{}'), step, created_at
FROM eval_metrics;

DROP TABLE eval_metrics;
ALTER TABLE eval_metrics_uuid RENAME TO eval_metrics;
...
CREATE TABLE IF NOT EXISTS policy_events_uuid (
    id TEXT PRIMARY KEY DEFAULT ('mvc_' || lower(hex(randomblob(16)))),
    event_type TEXT NOT NULL,
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT check_policy_events_id_mvc CHECK (id LIKE 'mvc_%')
);

INSERT INTO policy_events_uuid (
    id, event_type, status, details_json, subject_type, subject_id, policy_name,
    decision, reason, payload_json, created_at
)
SELECT
    CASE WHEN typeof(id) = 'text' AND id LIKE 'mvc_%' THEN id ELSE 'mvc_' || lower(hex(randomblob(16))) END,
    event_type, status, COALESCE(details_json, '{}'), subject_type, subject_id, policy_name,
    decision, reason, COALESCE(payload_json, '{}'), created_at
FROM policy_events;

DROP TABLE policy_events;
ALTER TABLE policy_events_uuid RENAME TO policy_events;
