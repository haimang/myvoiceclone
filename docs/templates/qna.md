...
> - 如采用 **身份反转版**，必须在文首显式声明角色对调关系。典型格式是：`某审查者/架构师提出问题 + 当前建议 + Reasoning；GPT 填 second opinion；业主裁决`。此时只替换 second-opinion 槽位的角色名，不得删除 `影响范围 / 为什么必须确认 / 当前建议 / Reasoning / second opinion / 问题 / 业主回答` 这些结构字段。
...
- 必须在文首用显式说明写清：**谁负责“当前建议 + Reasoning”，谁负责 second opinion，业主如何填写**
- 身份反转只改变“谁给 second opinion”，**不改变 QNA 的信息密度与治理地位**
- 若标准版写作是 `GPT 当前建议 + Opus second opinion`，身份反转版可以改为 `Opus 当前建议 + GPT second opinion`；但二者都必须保留同等完整的决策材料
- 冻结后，引用方只看 `Q 编号 + 业主回答`，不在其他文档重复抄写双方分析
